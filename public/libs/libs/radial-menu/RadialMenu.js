...
function RadialMenu(params) {
    ...
    self.radius      = 50;
		...
		if(params.step === 1) {
			self.innerRadius = self.radius * 0.47;
		} else if (params.step === 2) {
			self.innerRadius = self.radius * 0.52;
		} else {
    ...
    self.createHolder(params.step ?? 0);
    self.addIconSymbols();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
RadialMenu.prototype.createHolder = function (step) {
    var self = this;

		var stepClassName = 'step'+step
    self.holder = document.createElement('div');
    self.holder.className = 'menuHolder ' + stepClassName;
...
RadialMenu.prototype.createMenu = function (classValue, levelItems, nested) {
    ...
    var angleShift  = angleStep / 2 + 180; // 메뉴7개
		if (this.menuItems.length === 6) {
			angleShift  = angleStep / 2 + 150;		// 메뉴 6개 정상미포함
		}
    ...
    svg.addEventListener('mousedown', function (event) {
		...
		// 서클 모서리 클릭 방지
		svg.querySelectorAll('.sector').forEach(g => {
...
RadialMenu.prototype.appendSectorPath = function (startAngleDeg, endAngleDeg, svg, item, index) {
    ...
    if (item) {
        g.setAttribute('class', 'sector');
				g.setAttribute('fill', item.color);
        // if (index == 0) {
        ...
        if (item.title) {
            ...
            if (item.icon) {
                ...
                text.setAttribute('transform', 'translate(0,-3)');
            }
            ...
            g.appendChild(text);
				...
				if (item.subTitle) {
					var subText = self.createText(centerPoint.x, centerPoint.y, item.subTitle);
					subText.setAttribute('transform', 'translate(0,5)');
