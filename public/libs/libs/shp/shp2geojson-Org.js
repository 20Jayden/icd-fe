var inputData = {},
	...
	encoding,
	EPSG,
	EPSGOut = proj4('EPSG:3857')

function loadshp(config, returnData) {
	url = config.url
	encoding = typeof config.encoding != 'utf-8' ? config.encoding : 'utf-8'
	EPSG = typeof config.EPSG != 'undefined' ? config.EPSG : 3857
	var toEPSG = typeof config.toEPSG != 'undefined' ? config.toEPSG : 3857
	EPSGOut = proj4('EPSG:' + toEPSG)
		...
		reader.onload = function (e) {
			...
			zip.loadAsync(e.target.result).then(function (content) {
				...
				zip
					...
					.async('arraybuffer')
					.then(function (data) {
						SHPParser.load(URL.createObjectURL(new Blob([data])), shpLoader, returnData)
					})
				zip
					.file(dbfString)
					.async('arraybuffer')
					.then(function (data) {
						DBFParser.load(URL.createObjectURL(new Blob([data])), encoding, dbfLoader, returnData)
					})
			})
		}
...
function TransCoord(x, y) {
	if (proj4) var p = proj4(EPSGUser, EPSGOut, [parseFloat(x), parseFloat(y)])
	return { x: p[0], y: p[1] }
}
		...
		switch (shpRecords[i].shape.type) {
			...
			case 5:
				...
				for (var pts = 0; pts < shpRecords[i].shape.content.parts.length; pts++) {
					...
					for (
						var j = partsIndex * 2;
						j < (shpRecords[i].shape.content.parts[pts + 1] * 2 || shpRecords[i].shape.content.points.length);
						j += 2
					) {
