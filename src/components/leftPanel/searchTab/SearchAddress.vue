<template>
	<li :class="{ 'active': active }">
		<a @click="$emit('onActiveChange', 'address')"><span>주소</span></a>
		<div class="tab-pane">
			<!-- <div class="guide-ment"><strong>검색</strong> 또는 <strong>지도</strong>에서 지점 선택</div> -->
			<div class="guide-ment"><strong>주소명</strong>으로 검색</div>
			...
			<div class="result-list">
				...
				<ul v-else>
					<li v-for="(data, index) in searchResult" :key="index" @click="onSearchResultClick(data)">
						<div class="info">
							<strong class="name">{{ data.address_name }}</strong>
							<span class="address">{{ getAddressTypeKor(data.address_type) }}</span>
						</div>
						<button type="button" class="btn btn-marker"></button>
					</li>
				</ul>
...
export default {
	...
	setup() {
		...
		let searchText = ref('') // 검색어
		let searchResult = ref([]) // 검색결과
		...
		const search = async () => {
			...
			// 카카오 검색 API Request -> Response
			const res = await $kakao.get('/v2/local/search/address.json', {
				params: {
					page: currPage || 1,
					size: countPerPage,
					sort: 'accuracy',
			...
			if (res.status === 200) {
				CommonUtil.log(log.TARGET.ADDRESS, log.ACTION.SUCCESS, null)
			} else {
				CommonUtil.log(log.TARGET.ADDRESS, log.ACTION.FAIL, null)
			}
		}
		...
		const onSearchResultClick = async (item) => {
			...
			overlayStore.mergeState({
				...
				category: getAddressTypeKor(item.address_type),
