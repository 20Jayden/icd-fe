<template>
	<div class="search">
		<div class="tit">실거래 모니터링 리스트</div>
	...
	<Teleport to="#popupContainer">
		<RtmsMonitoringCondition @search="onSearch" />
		<RtmsSummaryDetail
			v-if="isActiveDetail"
...
import { rtmsMonitoring as monitoringApi, monitorBottom as pnuApi } from '@/api/monitor'
import locationApi from '@/api/location'
import { useUtil } from '@/composition/util'
import { useLoadingStore } from '@/stores/loading'
import { useOverlayStore } from '@/stores/overlay'
import { useUserStore } from '@/stores/user'
import RtmsMonitoringCondition from '@/components/popup/monitoringCondition/RtmsMonitoringCondition.vue'
...
export default {
	...
	setup() {
		...
		const searchList = async (pageNum = 1, payload = latestPayload) => {
			...
			if (status.code == 200) {
				CommonUtil.log(log.TARGET.RTMSMONITORING, log.ACTION.SUCCESS, null)
			} else {
				CommonUtil.log(log.TARGET.RTMSMONITORING, log.ACTION.FAIL, null)
			}
		...
		const createSummaryOverlay = (item) => {
			...
			div.classList.add('lv-' + numberToLevel(item.objAmtAvrg))
			...
			span.innerHTML = `${new Intl.NumberFormat().format(item.dealCnt || 0)}건/${tmpAmt}억`
